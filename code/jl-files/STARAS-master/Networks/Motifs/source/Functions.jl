...
function single_attack(nwork::LightGraphs.Graph,nw_vert_empty::Array{node,1}, nw_mat::Array{Float64,2},chosen::Int64,ant_to_node::Float64)
    ...
    return push!([sum([i["S"]+i["R"] for i in nw_res])]/node_num,sum([i["S"] for i in nw_res]), sum([i["R"] for i in nw_res]), (quantify_wins(nw_res)/node_num)..., density(nwork), global_clustering_coefficient(nwork), closeness_centrality(nwork)[chosen], degree(nwork,chosen), betweenness_centrality(nwork)[chosen],ant_to_node)
end
