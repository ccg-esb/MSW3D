...
function create_nw(ntype::String,node_num::Int,mean_edges::Int,chan_len::Float64,ant_to_node::Float64,r=1)
    nwork = ntype == "erdos" ? nets[ntype](node_num, node_num*mean_edges) : nets[ntype](node_num,mean_edges,watts_beta)
    ...
    chosen =  r == 1 ?  rand(1:node_num) : search_hub(nw_mat) # Search for the most connected Or choose a random node
...
function sim_randden(ntype::String)
    rnum = rand(edge_range,1)[1]
    nwork,nw_vert, nw_mat = create_nw(ntype,node_num,rnum,chan_len,ant_to_node)
    nw_res = sim_network(nw_vert,nw_mat)
    return push!([sum([i["S"] for i in nw_res])], sum([i["R"] for i in nw_res]), quantify_wins(nw_res)...,nwork.ne, density(nwork), global_clustering_coefficient(nwork))
end

function nsim_watts_rden(n::Int64)
    for i=1:n
        for j in sim_randden("watts")
            print("$j\t")
        end
        print("\n")
    end
