...
function create_nw(ntype::String,node_num::Int,mean_edges::Int,chan_len::Float64,ant_to_node::Float64, ant_source::String)
    ...
    return nwork, nw_vert, nw_mat, betweenness_centrality(nwork)[chosen]
...
function sim(ntype::String,ant_source::String)
    nwork,nw_vert, nw_mat,bet_centr = create_nw(ntype,node_num,mean_edges,chan_len,ant_to_node,ant_source)
    nw_res = sim_network(nw_vert,nw_mat)
    return  push!([sum([i["S"] for i in nw_res])], sum([i["R"] for i in nw_res]), quantify_wins(nw_res)..., density(nwork), global_clustering_coefficient(nwork),bet_centr)
end
