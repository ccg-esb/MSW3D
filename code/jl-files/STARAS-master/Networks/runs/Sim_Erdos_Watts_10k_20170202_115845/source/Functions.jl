...
function iter(d0,con_mat)
    ...
    for i in time
        #Reaction
            ...
            data[j] = newdata[2][end]
        end

        #Difussion
        temp = copy([data[m][4] for m=1:n])
        for k = 1:n-1
            for l = k+1:n
                data[k][4] += con_mat[k,l] * ant.delta * (temp[l] - temp[k])
                data[l][4] += con_mat[k,l] * ant.delta * (temp[k] - temp[l])
            end
        end
    end
    return data
...
function sim_network(vert,mat)
    ...
    return [Dict("S"=>i[2],"R"=>i[3]) for i in res]
end

function sim(ntype)
    nwork,nw_vert, nw_mat = create_nw(ntype,node_num,mean_edges,chan_len,ant_to_node)
    nw_res = sim_network(nw_vert,nw_mat)
    return push!(convert(Array{Float64},quantify_wins(nw_res)),global_clustering_coefficient(nwork),diameter(nwork),radius(nwork))
end

function nsim_watts(n::Int64)
    return [sim("watts") for i=1:n]
end

function nsim_erdos(n::Int64)
    return [sim("erdos") for i=1:n]
end