...
function iter(d0,con_mat)
    ...
    for i in time
        ...
        for j=1:n
            ...
            @assert data[j][3] > -1 " Antibiotics gone wild"
        end
        #Difussion
...
function connect_components(nwork)
    while length(connected_components(nwork)) > 1
        ...
        @assert iter_counter<100 "Too many iterations to reconnect"
...
function create_nw(ntype::String, node_num::Int, edges::Int, chan_len::Float64, ant_to_node::Float64, r=1)
    ...
    return nwork,nw_vert, nw_mat,chosen
end
...
function sim_randden(ntype::String,rnum::Int)
    nwork,nw_vert, nw_mat,chosen = create_nw(ntype,node_num,rnum,chan_len,ant_to_node)
...
function nsim_watts_rden(n::Int64)
    rnum = convert(Array{Int64},round(rand(edge_range,n) ./ node_num))
    for i=1:n
        for j in sim_randden("watts",rnum[i])
            print("$j\t")
        end
        print("\n")
    end
...
function nsim_erdos_rden(n::Int64)
    rnum = rand(edge_range,n)
    for i=1:n
        for j in sim_randden("erdos",rnum[i])
            print("$j\t")
        end
        print("\n")
    end
