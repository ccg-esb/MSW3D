...
function create_watts_nw(node_num::Int,mean_edges::Int,Beta::Float64,chan_len::Float64,ant_to_node::Float64,r=1)
    nwork = nets["watts"](node_num,mean_edges,Beta)
...
function sim_watts_randB(rnum::Float64)
    nwork,nw_vert, nw_mat = create_watts_nw(node_num,mean_edges,rnum,chan_len,ant_to_node)
	nw_res = sim_network(nw_vert,nw_mat)
    return push!([sum([i["S"] for i in nw_res])], sum([i["R"] for i in nw_res]), quantify_wins(nw_res)...,rnum, density(nwork), global_clustering_coefficient(nwork))
end

function nsim_watts_randB(n::Int64)
    rnum = rand(watts_Brange,n)
    for i=1:n
        for j in sim_watts_randB(rnum[i])
            print("$j\t")
        end
        print("\n")
    end
