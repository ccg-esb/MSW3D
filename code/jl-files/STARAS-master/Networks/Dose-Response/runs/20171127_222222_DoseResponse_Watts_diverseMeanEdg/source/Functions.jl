...
function nsim_dr_erdos(n::Int64,mean_edges::Int64,wdir::String)
    for i=1:n
        for j in Sim_network_DR("erdos",mean_edges)
            for k in j
                print("$k\t")
            end
            print("\n")
    ...
    writedlm("$(wdir)/ANT/ant_$(mean_edges)edges_erdos_dist",transpose(reshape(ant_vector_tofile,node_num,n)))
end
...
function sim_empty_network(reps::Int64,mean_edges::Int64,wdir::String)
    ...
    writedlm("$(wdir)/ANT/ant_$(mean_edges)edges_watts_dist",ant_vector_tofile)
    writedlm("$(wdir)/ANT/ant_$(mean_edges)edges_erdos_dist",ant_vector_tofile)
end