...
function hub_attack(nwork::LightGraphs.Graph,nw_vert_empty::Array{node,1}, nw_mat::Array{Float64,2},ant_to_node::Float64,chosen::Int64,mean_edges::Int64)
...
function Sim_network_DR(ntype::String,mean_edges::Int64)
    ...
    node_to_attack = search_hub(nw_mat)
    result_vector = [hub_attack(nwork,deepcopy(nw_vert_empty),nw_mat,ant_con,node_to_attack,mean_edges) for ant_con in ant_range]
    return result_vector
end

function nsim_dr_watts(n::Int64,mean_edges::Int64)
    for i=1:n
...
function nsim_dr_erdos(n::Int64,mean_edges::Int64)
    for i=1:n
        for j in Sim_network_DR("erdos",mean_edges*node_num)
            for k in j
                print("$k\t")
            end
            print("\n")
...
function sim_empty_network(n::Int64,mean_edges::Int64)
    nwork = Graph()
    add_vertices!(nwork,n)
    nw_vert_empty = [create_node(i) for i in nwork.vertices]
    nw_mat = convert(Array{Float64},adjacency_matrix(nwork)) * chan_len
    node_to_attack = rand(1:n)

    for ant_con in ant_range
        for i in hub_attack(nwork,deepcopy(nw_vert_empty),nw_mat,ant_con,node_to_attack,mean_edges)
            print(i,"\t")
        end
    end
end